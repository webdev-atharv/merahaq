...
@media(max-width:768px){
  ...
  *{box-sizing:border-box}
  ...
  .rich-content{display:none !important}
...
function fixInline(html) {
  ...
  // Fix adsbygoogle empty box
  html = html.replace(/(style="display:block")(\s*data-full-width)/g, 'style="display:block;min-height:0"$2');

  // REMOVE rich-content section entirely (generic content not needed)
  html = html.replace(/<section class="rich-content">[\s\S]*?<\/section>/g, '');
  // REMOVE related-links section entirely
  html = html.replace(/<section class="related-links"[\s\S]*?<\/section>/g, '');
